...
class TestSafeTxHash:
    ...
    def test_setter_strips_0x_prefix(self, base_behaviour: ConcreteBehaviour) -> None:
    ...
    def test_setter_rejects_invalid_length(
        self, base_behaviour: ConcreteBehaviour
    ) -> None:
...
class TestMultisendProperties:
    ...
    def test_multi_send_txs_serializes_batches(
        self, base_behaviour: ConcreteBehaviour
    ) -> None:
    ...
    def test_txs_value_sums_batch_values(
        self, base_behaviour: ConcreteBehaviour
    ) -> None:
    ...
    def test_txs_value_empty_is_zero(self, base_behaviour: ConcreteBehaviour) -> None:
        """Test txs_value with no batches returns 0."""
        assert base_behaviour.txs_value == 0

    def test_tx_hex_raises_without_hash(
        self, base_behaviour: ConcreteBehaviour
    ) -> None:
...
class TestContractInteractionError:
    ...
    def test_logs_at_correct_level(
        self, base_behaviour: ConcreteBehaviour, body: Any, expected_level: Any
    ) -> None:
    ...
    def test_falls_back_to_default_error(
        self, base_behaviour: ConcreteBehaviour
    ) -> None:
    ...
    def test_default_error(self, base_behaviour: ConcreteBehaviour) -> None:
...
class TestShouldUseMarketplaceV2:
    ...
    def test_disabled_returns_false(self, base_behaviour: ConcreteBehaviour) -> None:
    ...
    def test_raises_when_check_not_performed(
        self, base_behaviour: ConcreteBehaviour
    ) -> None:
        ...
        type(base_behaviour).synchronized_data = property(lambda self: synced)  # type: ignore[method-assign,assignment]
        with pytest.raises(ValueError, match="Compatibility check"):
            base_behaviour.should_use_marketplace_v2()

    def test_returns_true_when_v2(self, base_behaviour: ConcreteBehaviour) -> None:
